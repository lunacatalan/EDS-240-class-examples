...
fracking_clean <- fracking |> 
  ...
  mutate(datetime_start = mdy_hms(job_start_date)) |> # added column to convert from string to date (save to new col)
  mutate(year = year(datetime_start)) |> # create 'year' col from date column 

  # select relevant cols ----
  select(datetime_start, year, state_name, county_name, well_name, total_base_water_volume) |> # select these columns 
  
  # filter out non-state names ----
  filter(!state_name %in% c("Beaver", "Beckham", # remove these rows from state_name column 
                            "Harper", "Hemphill", 
                            "Midland", "Red River", 
                            "Roosevelt", "Rusk", 
                            "State", "WARD")) |> 
  ...
  mutate(sn = str_to_title(sn)) |> # capitalize first letter, make everything else lowercase
  
  # fix misspelled state names ----
  mutate(sn = case_when(
    sn == "Colordao" ~ "Colorado", # change misspellings in sn column on the left to the correct spelling on the right
  ...
  filter(!str_detect(string = sn, # select the sn column 
                     pattern = "\\?")) |> # `?` is a special chr; escape with `\\` prefix
  ...
  mutate(state_abb = usdata::state2abbr(sn)) |> # add column with only abbreviations
  
  # rename 'sn' to 'state_name' again for clarity ----
  rename(state_name = sn, 
         total_base_water_volume_gal = total_base_water_volume) |> 
  
  # move 'state_abb' col after state_name col in the dataframe ----
...
The thinner it is, the less data there is at those values.

Questions
- how is data distributed in each state?
- how is the distribution of data different across these states?
